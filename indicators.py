...
def williams_r(data, periods=14, high_col='<HIGH>', low_col='<LOW>', close_col='<CLOSE>'):
    ...
    for index,row in data.iterrows():
        if index > periods:
            data.set_value(index, 'williams_r', ((max(data[high_col][(index-periods-1):(index+1)]) - row[close_col]) / 
                                                 (max(data[high_col][(index-periods-1):(index+1)]) - min(data[low_col][(index-periods-1):(index+1)]))))
